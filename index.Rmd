...
The first tool I use to visualise the traffic pattern is `autoplot`.

explain the code

`autoplot` automatically gives you a conventional time series line plot. Bc `autoplot` recognises
the input as a `ts` class. So it knows how to deal with it by plotting the obs
against the time index. As you can see, different patterns for differnt locations.

`autoplot` gives a nice line graph due to the `ts` object, but it also becomes a 
limiting factor to try other types of plots because of the `ts` object.
...
Let's take a closer look at the `ts` class.

The `class` function tells us what class the object is.

Rather than a matrix, the `tsp` attributes stored as meta info make it to a `ts` object.

The advantage for using a `ts` data class is 1.

The second is as the time attributes are stored as metadata, for some specialist
methods like, they know how to automatically handle them 

However, `ts` class isn't data centric, not flexible for data analysis and vis.

hard to handle missing values

It's certainly not my goal to teach you how to use `autoplot`. We're going to
do more that beyond what `autoplot` can do.
...
unexpected and use your expertise or other resouces to explain and produce another
graph.
...
should be conducted under the statitical hyphothesis testing framework, known
as visual inference. 
