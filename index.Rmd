...
the data wrangling to get the tidy temporal data. Second, we'll learn about the 
...
I'll use three datasets to explain what tidy data actually mean.
...
As you can see, different temporal patterns for different locations. You may think
why `autoplot` gives you a conventional time series line plot instead of other
types of plots like dot plots. Bc `autoplot` is an S3 method and it recognises
...
Instead of storing the time index as a column, it's being stored as meta info
which is `tsp` attribute.
